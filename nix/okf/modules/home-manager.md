...
Agent applications are selected explicitly from the locked `llm-agents.nix` input. The `mcp-servers-nix` Home Manager module contributes packaged servers to `programs.mcp.servers`; remote and unsupported servers are added directly to that same registry. Codex, OpenCode, and Antigravity consume the shared registry, and GitHub credentials are scoped to the GitHub MCP server wrapper.
