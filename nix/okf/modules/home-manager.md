...
timestamp: 2026-08-13T00:00:00+06:00
---
...
The layer sets `home.username`, `home.homeDirectory`, default editor session variables, the Home Manager state version, and Catppuccin theme defaults. It enables common modules by default, including Bash, CLI tooling, direnv, Codex, git, Helix, Nushell, OpenCode, nixd, Spec Kit, Yazi, and Zellij. Common CLI utilities include the DuckDB CLI.

Agent applications are selected explicitly from the locked `llm-agents.nix` input. The `mcp-servers-nix` Home Manager module contributes packaged servers to `programs.mcp.servers`, while hosted servers are added directly to that same registry. Local MCP commands use packaged store binaries rather than runtime dependency resolvers. Codex uses Home Manager's canonical MCP transformation as input to a tested merge utility that preserves a writable, user-owned `config.toml`. OpenCode also consumes the shared registry, Spec Kit provides the `specify` CLI, and GitHub credentials are scoped to the GitHub MCP server wrapper.
