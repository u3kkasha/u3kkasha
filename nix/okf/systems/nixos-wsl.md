...
The `nixos-wsl` host enables the shared system layer, Podman, and the repository's WSL module. It sets `networking.hostName` to `nixos-wsl` and `nixpkgs.hostPlatform` to `x86_64-linux`.
